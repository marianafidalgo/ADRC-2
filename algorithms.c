  ...
  if(question == 6){
    ...
    for(int i = 1; i < MAX_NODES; i++){
      if(n == graph->num_V)
        break;
      if(graph->a_list[i] != 0){
        BGP_shortest(graph, queue, i, length, curr_type, final_length, src, dest, question);
        n++;
      }
    }
  }
  else{
    ans = BGP_shortest(graph, queue, dest, length, curr_type, final_length, src, dest, question);
  }
...
int BGP_shortest(struct Graph * graph, struct Queue * queue, int src, int * length, int * curr_type, int * final_length, int source, int dest, int question){
  ...
  int asked_type = 0;
  int a = 0;
  int *queue_lengths = (int*)malloc(MAX_NODES*sizeof(int));
  int *queue_types = (int*)malloc(MAX_NODES*sizeof(int));

  push_queue(queue, src);
  queue_types[queue->head] = -1;
  queue_lengths[queue->head] = 0;
  curr_type[src] = -1;
  length[src] = 0;
  ...
  while (queue->count != 0){
    ...
    while(temp){
      ...
      // printf("no provider pop type %d\n", queue_types[queue->head-1]);
      // printf("no provider type %d\n", curr_type[temp->name]);
      if(temp->name != src && (queue_types[queue->head-1] == -1 || queue_types[queue->head-1] == 1) && (curr_type[temp->name] == 0 || curr_type[temp->name] > 1)){
        // if(length[id_pop] + 1 < length[temp->name])
        //   length[temp->name] = length[id_pop] + 1;
    ...
    while(temp){
      ...
      // printf("no peer type %d\n", curr_type[temp->name]);
      if(curr_type[temp->name] == 0 && temp->name != src && (queue_types[queue->head-1] == -1 || queue_types[queue->head-1] == 1)){
      // printf("queue len %d\n", queue_lengths[queue->head-1]);
      // printf("temp len %d\n\n",length[temp->name]);
        if(queue_lengths[queue->head-1] + 1 < length[temp->name])
          length[temp->name] = queue_lengths[queue->head-1] + 1;
    ...
    if(queue->array[i] == 0)
     ...
     // printf("sou o %d\n", queue->array[i]);
      while(temp){
        // printf("sou o filho %d\n", temp->name);
        // printf("tenho %d\n", length[temp->name]);
        if(temp->name != src /*&& ((length[queue->array[i]] + 1) < length[temp->name])/*&& length[temp->name] == MAX_NODES*/){
          node = createNode(temp->name, 3);
          graph->visited[temp->name] = 1;
          a = length[queue->array[i]]  + 1;
          length[temp->name] = length[queue->array[i]]  + 1;
         // printf("a %d %d\n",a, temp->name);
          if(graph->bgp_clients[a] == NULL)
             graph->bgp_clients[a] = node;
          else{
  ...
  for (int i = 1; i < MAX_NODES; i++){
    ...
    while(bgp_temp){
      ...
      if(length[bgp_temp->name] > i){
        //printf("entrei bgp %d\n", bgp_temp->name);
        length[bgp_temp->name] = i;
        graph->visited[bgp_temp->name] = 1;
        ...
        while(temp){
          // printf("sou o bgp filho %d\n", temp->name);
          // printf("tenho %d\n", length[temp->name]);
          if(temp->name != src /*&& ((length[bgp_temp->name] + 1) < length[temp->name])*/){
           // printf("cria no %d\n", temp->name);
            node = createNode(temp->name, 3);
            graph->visited[temp->name] = 0;
            a = length[bgp_temp->name] + 1;
            //printf("a  %d\n", a);
            if(graph->bgp_clients[a] == NULL)
              graph->bgp_clients[a] = node;
